...
export interface IUser {
    ...
    //without a password value. we will validate that it is not empty when creating a user
    password? : string;
    role: Roles;
}

//Our AP supports both an admin and regular user, as defined by a role. 
 export enum Roles {
    ADMIN = 'ADMIN',
    USER = 'USER'
 }

 //lets initialize our example API with some user records. 
 //Note: we generate passwords using the Node JS CLI with thus command: 
